...
import edu.columbia.cs.ref.tool.tagger.entity.impl.resources.ParserAnnotations.ConstraintAnnotation;
...
public class RuleBasedCorefMentionFinder implements CorefMentionFinder {
	...
	private Map<Integer, Entity> entityTable;

	public RuleBasedCorefMentionFinder(List<Entity> currentEntities, Map<Integer, Entity> entityTable) {
	...
	private void extractNamedEntityMentions(CoreMap s, List<Mention> mentions, Set<IntPair> mentionSpanSet, Set<IntPair> namedEntitySpanSet, List<Entity> entities, Map<Integer,Entity> mentionTable) {
		...
		for (int i = 0; i < entities.size(); i++) {
			...
			int offset = entities.get(i).getOffset();

			//see if we are in the right sentence

			boolean isLocation = entities.get(i).getEntityType().equals("LOCATION");
			
			if (sent.get(0).get(CharacterOffsetBeginAnnotation.class) <= offset){ //sentence starts before the entity

				int lenght = entities.get(i).getLength();

				if (sent.get(sent.size()-1).get(CharacterOffsetEndAnnotation.class) >= offset+lenght){ //sentence finishes after the entity 

					int auxBeg = binarySearchFind(offset,sent,0,sent.size()-1);

					beginIndex =  auxBeg;

					endIndex = auxBeg + SeqSearchFind(offset+lenght-1,sent.subList(auxBeg, sent.size())) + 1; //because it starts in 1 and because it's the following after it finishes

					//TODO see if I need to ask for 's and endIndex++;
					...
					entityTable.put(mentionId, entities.get(i));
					...
					//for USE_DIFFERENT_LOCATION in DeterministicCorefSieve line 278
					
					if (isLocation){
						for (int j = beginIndex; j < endIndex; j++) {
							sent.get(j).setNER("LOCATION");
						}
					} 
		...
		for (int i = 0; i < sent.size(); i++) {
			...
			if (!sent.get(i).containsKey(NamedEntityTagAnnotation.class)){
				sent.get(i).setNER("-");
			}
	...
	private int binarySearchFind(int offset, List<CoreLabel> sent, int imin, int imax) {
		...
		while (imax >= imin)
		{
			...
			if    	(sent.get(imid).get(CharacterOffsetEndAnnotation.class) <  offset)
				// change min index to search upper subarray
				imin = imid + 1;
			else if (sent.get(imid).get(CharacterOffsetBeginAnnotation.class) > offset )
	...
	private int SeqSearchFind(int offset, List<CoreLabel> list) {

		for (int i = 0; i < list.size(); i++) {

			if (list.get(i).get(CharacterOffsetBeginAnnotation.class) <= offset && offset <= list.get(i).get(CharacterOffsetEndAnnotation.class)){
				return i;
			}
...
//my comment			String headNE = m.headWord.get(NamedEntityTagAnnotation.class);
			// pleonastic it
...
//my comment			if(headNE.equals("PERCENT") || headNE.equals("MONEY")) remove.add(m);
	...
	public List<List<Mention>> extractPredictedMentions(Annotation doc,
			int _maxID, Dictionaries dict) {
		this.maxID = _maxID;
		List<List<Mention>> predictedMentions = new ArrayList<List<Mention>>();
		for(CoreMap s : doc.get(SentencesAnnotation.class)) {
			...
			extractNamedEntityMentions(s, mentions, mentionSpanSet, namedEntitySpanSet,currentEntities,entityTable);
